...
def camera_thread_loop(user):
    ...
    timeout_limit = 15  # The camera will actively scan for 15 seconds in the background
...
#         if is_tty:
#             # TERMINAL MODE: Run your parallel input engine
#             prompt = f"[sudo] password for {user} (or scanning face...): "
#             password_attempt = get_parallel_input(prompt)
#         else:
#             # GUI/LOCKSCREEN MODE: Bring up the input field instantly!
#             # The camera thread will run simultaneously in the background for its 15s limit.
#             try:
...
def pam_sm_authenticate(pamh, flags, argv):
    ...
    try:
        ...
        if is_tty:
            ...
            # GUI/LOCKSCREEN MODE: Fast non-blocking poll window
            # Gives the camera 2 seconds to scan your face hands-free before loading the GUI box
            gui_poll_limit = 2.0  
            start_scan = time.time()
            while (time.time() - start_scan) < gui_poll_limit:
                if face_matched:
                    break
                time.sleep(0.05)

            # If the face didn't match within 2 seconds, pop up the graphical prompt box safely.
            # Note: The background thread keeps scanning for up to 15s total while this box sits open.
            if not face_matched:
                try:
                    resp = pamh.conversation(pamh.Message(pamh.PAM_PROMPT_ECHO_OFF, "Password (or scanning face...): "))
                    password_attempt = resp.resp
